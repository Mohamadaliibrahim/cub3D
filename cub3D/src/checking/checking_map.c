...
/*   Updated: 2024/11/17 23:58:10 by mohamibr         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	parse_color_line(char *line, t_config *confige)
{
	...
	else if (!strcmp(parts[0], "C"))
		confige->ceiling_color = (red << 16) | (green << 8) | blue;
	else
	{
		write_error("Error: Unkown color identifier\n");
		two_d_free(parts);
		two_d_free(colors);
		free_list(confige);
		return ;
